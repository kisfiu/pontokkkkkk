...
public class NeurMenuActivity extends android.app.Activity 
{
    ...
    public void onCreate(android.os.Bundle savedInstanceState)
	{
        ...
        setContentView(R.layout.menu_neuron);
		...
		{ 
			...
			{ 
				...
				android.widget.ImageView iv = (android.widget.ImageView) findViewById(R.id.neur_anim);
				iv.setBackgroundResource(R.drawable.neuron_animation);
				iv.setX(MENU_CIRCLE_MID_X - NEURON_SIZE / 2);
				iv.setY(MENU_CIRCLE_MID_Y - NEURON_SIZE / 2);
	...
	public int collapse()
	{
		float x = menuItems.get(activeIndices.get(activeIndices.size()-1)).getX();
		float y = menuItems.get(activeIndices.get(activeIndices.size()-1)).getY();
		...
		for(int i = 0; i<menuItems.size(); i++)
		{
			...
			if (tmp != 1)
			{
				float x_ = menuItems.get(i).getX();
				float y_ = menuItems.get(i).getY();
			
				if (Math.abs( x - x_ ) < MENU_ITEM_SIZE *2 && Math.abs( y - y_ ) < MENU_ITEM_SIZE * 2)
				{
					activeIndices.add(i);
					return i;			
				}
    ...
    public boolean onTouchEvent(android.view.MotionEvent event)
	{
		...
		if (event.getAction() == android.view.MotionEvent.ACTION_DOWN)
		{
			...
			activeIndices.clear();
			if(getNearestMenuItem(x,y) != -1)
			{
				activeIndices.add( getNearestMenuItem(x,y) );
			}
		}
		...
		else if (event.getAction() == android.view.MotionEvent.ACTION_MOVE)
		{
			...
			if (!activeIndices.isEmpty())
			{
				...
				for(int i = 0; i<activeIndices.size(); i++)
				{
					...
					Vector2d v = new Vector2d(menuItems.get(activeIndices.get(i)).getX() + MENU_ITEM_SIZE / 2 - MENU_CIRCLE_MID_X,
						menuItems.get(activeIndices.get(i)).getY() + MENU_ITEM_SIZE / 2 - MENU_CIRCLE_MID_Y);
					...
					menuItems.get(activeIndices.get(i)).setX( newCoords.getX() - MENU_ITEM_SIZE / 2 );
					menuItems.get(activeIndices.get(i)).setY( newCoords.getY() - MENU_ITEM_SIZE / 2 );
					//if(i == 12) break;
				}	
